                    ...
                    if (subStructure.LevelOfDetail <= lod || insideRigidBody)
                    {
                        ...
                        if (subStructure.LoadedGameObject == null)
                        {
                            GameObject gameObject =
                                Game.Instance.ResourceLoader.InstantiatePrefab(subStructure.PrefabPath);
                        ...
                        Traverse.Create(__instance).Method("InstantiateSubStructures").GetValue(
                            subStructure.LoadedGameObject.transform, subStructure.SubStructures, lod, insideRigidBody2);
                        if (Game.InFlightScene && flag && !insideRigidBody && subStructure.Mass > 0.0)
                        {
                            subStructure.LoadedGameObject.AddComponent<SubStructureRigidBodyScript>()
                                .Initialize(subStructure);
                        }
                    }
                    else if (subStructure.LoadedGameObject != null)
                    {
                        Traverse.Create<StructureNode>().Method("UnloadSubstructureGameObjects")
                            .GetValue(subStructure, true);
                    }
                }
            }
            catch (Exception e)
